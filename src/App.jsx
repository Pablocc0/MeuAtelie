    ...
    <div className="settings-group"><h3>Dados dos orçamentos</h3>
      <div className="studio-logo-setting">
        ...
        <div className="studio-logo-actions">
          <button type="button" className="settings-row" disabled={savingLogo} onClick={() => logoRef.current.click()}><Upload /><span><b>{savingLogo ? 'Salvando...' : data.profile.logo ? 'Trocar logo' : 'Adicionar logo'}</b><small>Escolher imagem da galeria</small></span><ChevronRight /></button>
          {data.profile.logo && <button type="button" className="settings-row studio-logo-remove" disabled={savingLogo} onClick={() => { saveProfile({ logo: '' }); notify('Logo removida') }}><Trash2 /><span><b>Remover logo</b></span></button>}
        </div>
